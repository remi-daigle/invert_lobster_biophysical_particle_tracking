...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
 ...
 SUBROUTINE opa_proj 
      ...
      integer :: i1,i2,j1,j2
      integer,parameter :: ijr=30 !Search radius around guessed point: Might need to be increase
...
!
       ...
       do j=j1,j2          
        ...
        !print*,'nav_lon,nav_lat =',nav_lon(i00,j00),nav_lat(i00,j00)
...
!       print*,r1,r2,Dx,Dy,dx1,dy1,ri00,rj00  
 ...
 SUBROUTINE opa_proj_inv 
      IMPLICIT NONE
      REAL :: lonCC,latCC,lonCL,latCL,lonTC,latTC,lonCR,latCR,lonBC,latBC
      REAL :: r1,r2,Dx,Dy,dx1,dy1,dri00,drj00,dlat_x,dlon_x,dlat_y,dlon_y
...
!     Find nearest wet i,j to lon00,lat00 ( +1 because it's atart to zero)
       i00=floor(ri00)+1
       j00=floor(rj00)+1

!      Finding the fraction in the cell ( -0.5 because ij start to the BL coner and lat/lon is in the center)
       dri00=ri00-1.0*(i00-1)-0.5
       drj00=rj00-1.0*(j00-1)-0.5

       ! use the neighbours cell to calcul the base vector in lat/lon for this cell 
       lonCC=nav_lon(i00,j00)
       latCC=nav_lat(i00,j00)
       lonCL=nav_lon(i00-1,j00) ! center-left
       latCL=nav_lat(i00-1,j00)
       lonTC=nav_lon(i00,j00+1) ! top-center
       latTC=nav_lat(i00,j00+1)
       lonCR=nav_lon(i00+1,j00) ! center-right
       latCR=nav_lat(i00+1,j00)
       lonBC=nav_lon(i00,j00-1) ! bottom-center
       latBC=nav_lat(i00,j00-1)

!      the delta_coor in the x/y direction
       dlat_x= (latCR-latCL)/2
       dlon_x= (lonCR-lonCL)/2
       dlat_y= (latTC-latBC)/2
       dlon_y= (lonTC-lonBC)/2

       lon00=lonCC+dlon_x*(dri00)+dlon_y*(drj00)
       lat00=latCC+dlat_x*(dri00)+dlat_y*(drj00)
